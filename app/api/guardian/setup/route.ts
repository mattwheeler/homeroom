  ...
  return handleGuardianSetup(request, {
    ...
    read: async (session) => {
      ...
      return buildGuardianWorkspace(
        stored ?? { settings: defaultGuardianSetupSettings(), settingsVersion: 0, updatedAt: null },
        sources,
    ...
    save: async (_session, input) => {
      ...
      return buildGuardianWorkspace(stored, await sourceStatuses(_session.studentId ?? _session.actorId), {
        guardianId: _session.actorId,
        studentId: _session.studentId ?? _session.actorId
      });
    }
