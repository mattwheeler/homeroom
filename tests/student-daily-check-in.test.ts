...
describe("bounded evidence-backed student check-in", () => {
  ...
  it("lets Emily talk immediately, offers editable focus starters, and progressively discloses alternatives", () => {
    ...
    expect(html).toContain("Conversation starters");
    ...
    expect(html).toContain("Choose one if it helps");
    expect(html).toContain("Talk to Homeroom");
    expect(html).not.toContain("disabled=\"\" placeholder=\"Choose how your focus feels first.");
  });

  it("accepts one authoritative primary task without repeating its recommendation", () => {
    const html = renderToStaticMarkup(createElement(StudentDailyCheckIn, {
      studentName: "Emily",
      projection: navigationProjection,
      now: new Date("2026-08-17T14:00:00.000Z"),
      primaryContent: createElement("div", null, "Unified primary task")
    }));

    expect(html).toContain("Good morning, Emily.");
    expect(html).toContain("Unified primary task");
    expect(html).toContain("Talk it through with Homeroom");
    expect(html).toContain("Something else…");
    expect(html).not.toContain("Recommended next step");
    expect(html).not.toContain("Show 3 other choices");
    expect(html).not.toContain("I am feeling confident and ready to get started today.");
    expect(html).not.toContain("<textarea");
    expect(html.indexOf("Good morning, Emily.")).toBeLessThan(html.indexOf("Talk it through with Homeroom"));
    expect(html.indexOf("Talk it through with Homeroom")).toBeLessThan(html.indexOf("Unified primary task"));
  });
});
