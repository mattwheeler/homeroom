...
describe("secure read-only source HTTP handlers", () => {
  ...
  it("starts Google OAuth and emits a short-lived callback-only Lax cookie", async () => {
    ...
    expect(response.headers.get("set-cookie")).toContain("Path=/api/integrations/google/callback");
  });
  ...
  it("connects a bounded BAND URL and manually syncs either provider", async () => {
    ...
    const synced = await handleSourceSync(
      await postRequest("/api/integrations/sync", { provider: "band_ical" }),
      { ...base(), sync }
    );
    ...
    expect(sync).toHaveBeenCalledWith(session(), { provider: "band_ical" });
  });
  ...
  it("completes the OAuth callback with one-time state and clears the callback cookie", async () => {
    ...
    const response = await handleGoogleClassroomCallback(
      ...
      { store: new MemorySessionStore(session()), signingSecret, now, complete }
    );
    expect(response.status).toBe(303);
    expect(response.headers.get("location")).toBe("https://homeroom.example/?source=google-connected#sources");
  ...
  it("rejects failed callbacks without leaking provider details", async () => {
    const failedByGoogle = await handleGoogleClassroomCallback(
      ...
      { store: new MemorySessionStore(session()), signingSecret, now, complete: vi.fn() }
    );
    expect(failedByGoogle.status).toBe(303);
    expect(failedByGoogle.headers.get("location")).toBe("https://homeroom.example/?source=google-declined#sources");

    const failedInternally = await handleGoogleClassroomCallback(
      await callbackRequest(`state=${"s".repeat(43)}&code=authorization-code`),
      { store: new MemorySessionStore(session()), signingSecret, now, complete: vi.fn().mockRejectedValue(new Error("refresh-secret")) }
    );
    expect(failedInternally.headers.get("location")).toBe("https://homeroom.example/?source=google-error#sources");
    expect(await failedInternally.text()).not.toContain("refresh-secret");
  });

  it("rejects missing auth, guardians, cross-origin, invalid CSRF, extra fields, and rate limits", async () => {
    const start = vi.fn();
    await expect(handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}, { includeCookie: false }),
      { ...base(), start }
    ...
    await expect(handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}, { origin: "https://attacker.example" }),
      { ...base(), start }
    )).resolves.toMatchObject({ status: 403 });
    await expect(handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}, { csrf: "bad" }),
      { ...base(), start }
    )).resolves.toMatchObject({ status: 403 });
    await expect(handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", { role: "owner" }),
      { ...base(), start }
    )).resolves.toMatchObject({ status: 400 });
    await expect(handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}),
      { ...base(), start, rateLimiter: { consume: () => false } }
    )).resolves.toMatchObject({ status: 429 });
    expect(start).not.toHaveBeenCalled();
  });

  it("requires the callback-only session cookie and a valid student session", async () => {
    ...
    await expect(handleGoogleClassroomCallback(
      await callbackRequest(`state=${"s".repeat(43)}&code=x`, { role: "guardian" }),
  ...
  it("bounds request parsing and safely maps source failures", async () => {
    const token = await signSessionToken({ sessionId: "session_01", role: "student", expiresAt }, signingSecret);
    const headers = {
    ...
    await expect(handleGoogleClassroomStart(invalidJson, { ...base(), start: vi.fn() }))
      .resolves.toMatchObject({ status: 400 });
    ...
    await expect(handleGoogleClassroomStart(oversized, { ...base(), start: vi.fn() }))
    ...
    await expect(handleGoogleClassroomStart(wrongContent, { ...base(), start: vi.fn() }))
      .resolves.toMatchObject({ status: 415 });

    const conflict = await handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}),
      { ...base(), start: vi.fn().mockRejectedValue(new SourceConnectionError("SOURCE_NOT_CONNECTED", "Not connected.")) }
    );
    expect(conflict.status).toBe(409);
    const failure = await handleGoogleClassroomStart(
      await postRequest("/api/integrations/google/start", {}),
      { ...base(), start: vi.fn().mockRejectedValue(new Error("provider-secret")) }
    );
  ...
  it("rejects missing, mismatched, and expired persisted sessions", async () => {
    const start = vi.fn();
    const request = await postRequest("/api/integrations/google/start", {});
    await expect(handleGoogleClassroomStart(request.clone(), { ...base(), store: new MemorySessionStore(null), start }))
      .resolves.toMatchObject({ status: 401 });
    await expect(handleGoogleClassroomStart(request.clone(), {
      ...base(),
      store: new MemorySessionStore({ ...session(), actorId: "guardian_matt" }),
      start
    })).resolves.toMatchObject({ status: 401 });
    await expect(handleGoogleClassroomStart(request.clone(), {
      ...base(),
      store: new MemorySessionStore({ ...session(), expiresAt: "2026-07-18T12:00:00.000Z" }),
      start
    })).resolves.toMatchObject({ status: 401 });
    expect(start).not.toHaveBeenCalled();
  });
